...
public class MailServiceImpl implements MailService {
    ...
    @Override
    public boolean sendMail(MailAction action, User user) throws MessagingException, MailException, IOException {
        MimeMessage mimeMessage = javaMailSender.createMimeMessage();
        MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, "utf-8");
        helper.setFrom(FROM);
        helper.setTo(user.getEmail());
        helper.setSubject(action.getEmailTitle());
        helper.setText(buildContent(action, user), true);
        //        javaMailSender.send(helper.getMimeMessage());
        return true;
    }

    private String buildContent(MailAction action, User user) throws IOException {
        FileInputStream fis = new FileInputStream("src/main/resources/template-user.html");
        return String.format(IOUtils.toString(fis, StandardCharsets.UTF_8),
    ...
    private String buildContent(MailAction action, Ride ride, User user) throws IOException {
        FileInputStream fis = new FileInputStream("src/main/resources/template-ride.html");
        return String.format(IOUtils.toString(fis, StandardCharsets.UTF_8),
                action.getContentTitle(),
                action.getContentSmallTitle(),
                convertStatus(ride.getStatus()),
                convertVehicle(vehicleMapper.toDto(user.getVehicles()
                        .stream()
                        .filter(v -> v.getId() == ride.getVehicleId())
